...
function _buildHWRBannerFromTrade(t) {
  try {
    ...
    const conf = t.conf != null ? t.conf
      : Math.max(0, 100 - calcLearnDrag(t.learnPenalty || 0) - (t.riskPenalty || 0));
    const adx  = t.adx || 20;
    const _W = [], _P = [];
    ...
    if (lp >= 20)      _W.push({ level:'danger',  text:`AI 止損風控觸發（扣 -${calcLearnDrag(lp)} 分）`, source:'AI 止損風控' });
    else if (lp >= 5)  _W.push({ level:'caution', text:`AI 止損風控警告（扣 -${calcLearnDrag(lp)} 分）`, source:'AI 止損風控' });
...
function buildOpenPositionSetup(t, currentPrice) {
  ...
  const conf     = t.conf != null ? t.conf
    : (t.learnPenalty != null
      ? Math.max(0, Math.round(100 - calcLearnDrag(t.learnPenalty) - (t.riskPenalty || 0)))
      : Math.max(0, 100 - calcLearnDrag(t.learnPenalty || 0) - (t.riskPenalty || 0)));
  ...
  const _openEffConf = t.conf != null ? t.conf : Math.max(0, 100 - calcLearnDrag(t.learnPenalty || 0) - (t.riskPenalty || 0));
...
function buildPendingPositionSetup(t, currentPrice) {
  ...
  const _effConf = t.conf != null ? t.conf : Math.max(0, 100 - calcLearnDrag(t.learnPenalty || 0) - (t.riskPenalty || 0));
  ...
  const _pLearnDrag = calcLearnDrag(t.learnPenalty || 0);
...
/* ── 限價單成交判定（忠實對應實盤限價單：碰到進場點即成交）─────────
   進場成交要如實反映使用者掛在交易所的限價單，不作為減量／提精準的手段
   （交易品質靠選單階段的 SQ／核心共識把關，與成交判定無關）。
   唯一保留的是「方向正確性」——用建單參考價（entryPrice）判斷價格該從
   哪一側回到進場點，只認該側的觸及：回踩單認低點觸及、突破單認高點觸及，
...
function isLimitFilled(trade) {
  ...
  if (ref <= 0) {
    // 舊單無建單參考價：退回區間穿越（曾涵蓋進場點即成交）
    return trade.loSince <= entry && trade.hiSince >= entry;
  }
  if (ref >= entry) return trade.loSince <= entry;   // 回踩：低點觸及進場點即成交
  return trade.hiSince >= entry;                     // 突破：高點觸及進場點即成交
}
...
function buildTradeSetup(coin, mtfData, deriv, globalMkt, whale, fearGreed) {
  ...
  if (existingActive) {
    ...
    if (!pendingCancelledByMacro) {
      ...
      if (existingActive.entryTime) {
        ...
        const _lockDrag = _learnPen != null ? calcLearnDrag(_learnPen) : 0;
      ...
      const confPanelHtml = `<div style="margin-top:12px;padding:10px 13px;background:rgba(255,255,255,.03);border:1px solid rgba(255,255,255,.07);border-radius:10px">
        ...
        <div style="font-size:0.72rem;line-height:1.8">
          ...
          ${learnResultNow.hardBlocked
            ...
            : learnPenNow > 0
              ? learnResultNow.warnings.map(w => `<div style="color:#f59e0b">⚠️ AI風控：${w}</div>`).join('') || `<div style="color:#f59e0b">⚠️ AI 風控：歷史規則觸發，扣 -${calcLearnDrag(learnPenNow)} 分</div>`
  ...
  const _cLearnDrag = calcLearnDrag(learnPenalty);
  ...
  _sqFactors.push(canScaleIn ? '✅ 週線+日線+4H 同向 + 折價區佈局（長線硬性條件）' : '✅ 4H+15m 雙確認（短線硬性條件）');
  {
    ...
    // 日線僅長線單計分：短線單不看日線（避免短線單因日線不同向被扣分/取消）
    if (canScaleIn) {
    ...
    } else              { _sqFactors.push('⬜ 日線不計分（短線單，僅長線需日線同向）'); }
  ...
  // ⑬ 風控評估 + 止損AI建議 — 僅顯示參考，不調整 SQ 分
  // （風控/止損已透過風控分扣分處理：高風險只扣風控分，與掃描三路徑的 SQ 評分原則一致）
  if (_risk.score <= 20)      { _sqFactors.push(`✅ 風控優良（${_risk.score}分）`); }
  else if (_risk.score <= 40) { _sqFactors.push(`✅ 風控良好（${_risk.score}分）`); }
  else if (_risk.score <= 54) { _sqFactors.push(`⚠️ 風控中等（${_risk.score}分）`); }
  else                        { _sqFactors.push(`❌ 風控偏高（${_risk.score}分，已於風控分扣分）`); }
  ...
  // 止損記錄警告 → 僅參考顯示，不扣 SQ 分（已由止損風控扣分處理）
  {
    ...
    if (_sqPenChecks.length) {
      _sqFactors.push(`⚠️ 止損記錄警告（${_sqPenChecks.length} 項，已於止損風控扣分）`);
...
function globalLossStreak() {
  // 快取 10 秒（saveTradeLog 寫入時主動失效）：此函式會在掃描迴圈的多個
  // 建單入口被逐幣呼叫，若每次都 JSON.parse 整份交易紀錄，紀錄一大
  // 會在掃描完成瞬間卡死主執行緒（頁面在倒數 58 秒左右當機的主因）
  const _c = globalLossStreak._cache;
  if (_c && Date.now() - _c.ts < 10000) return _c.val;
  ...
  const val = { streak, lastSlTime };
  globalLossStreak._cache = { ts: Date.now(), val };
  return val;
}
...
/* ── 從掃描數據自動記錄交易信號 ──────────────────────────────── */
/* ── SQ 監控評分（單一定義來源）──────────────────────────────────
   這是掛單監控使用的完整因子評分。建單前的「終審」與監控共用此函式：
   同一份數據算出同一個分數，從根本上杜絕「建單通過、下一輪監控取消」。
...
function computeSqMonitorScore(trade, _sqCoin, _sqIsLong, _ctx) {
  _ctx = _ctx || { wBias: 'neutral', tBias: 'neutral', btcChg24: NaN };
    ...
    // 日線僅長線單計分：短線單不看日線（避免短線掛單因日線不同向被監控降級/取消）
    if (trade.canScaleIn) { if (_rcDyOk)  _sqRC += 1; else if (_rcDyOpp) _sqRC -= 1; }
    ...
    try {
      if (_sqCoin.symbol !== 'BTC/USDT' && !isNaN(_ctx.btcChg24)) {
        const _rcRel = (parseFloat(_sqCoin.change24h) || 0) - _ctx.btcChg24;
    ...
    // ㉖ 實驗室驗證加權 ±2（與建單評分一致）
    try { const _rcLab = labTagSqBonus(_sqCoin, _sqIsLong, _ctx.btcChg24); _sqRC += _rcLab.delta; } catch(_e) {}
...
async function recordSignalsFromScan(data) {
  ...
  for (const coin of data) {
    ...
    try {
      const _scanPreConf = Math.max(0, 100 - calcLearnDrag(setup.learnPenalty || 0));
    ...
    } else               { _scanSqFactors.push('⬜ 日線不計分（短線單，僅長線需日線同向）'); }
  ...
  for (const trade of tlog) {
    ...
    // 加上取消門檻−2 分遲滯帶，時間緩衝不再需要，監控從第一輪就全程生效。
    ...
    if (!_rcSqPass) {
      ...
      if (!_sqCancelIds.has(trade.id)) {
        ...
        try {
          ...
          const _rcPreRiskConf = Math.max(0, 100 - calcLearnDrag(_rcLearnPen));
        ...
        const _rcConf = Math.max(0, 100 - calcLearnDrag(_rcLearnPen) - _rcRiskPen);
    ...
    try {
      ...
      for (const _pnt of _pendingNotify) {
        ...
        try {
          ...
          const _pntPreRiskConf = Math.max(0, 100 - calcLearnDrag(_pntLP));
...
/* ── 1m K棒插針驗證（補輪詢間隙）────────────────────────────────
   問題：updateOpenTrades 只用每輪掃描當下的瞬時價檢查止損/止盈，兩次輪詢
   之間的插針（wick）完全看不到 → 實體交易已被掃損，持倉頁卻顯示還活著。
   解法：每輪為持倉中/掛單中的交易抓 1m K 棒，用高低點逐棒檢查區間內是否
   實際觸及。同一根 K 棒同時觸及止損與止盈時，保守判定先觸止損。
   對象只有少數活躍單（上限 ~8 筆），每輪多 幾個輕量請求，不影響主掃描。 */
let _wickCheckBusy = false;
async function verifyIntrabarHits() {
  if (!isSignalMaster() || _wickCheckBusy) return;
  ...
  try {
    ...
    for (const trade of actives) {
      ...
      for (const bar of raw) {
        ...
        const hi = parseFloat(bar[2]), lo = parseFloat(bar[3]);
        const slNow = trade.sl;
        const slHit  = isLong ? lo <= slNow : hi >= slNow;
...
async function checkAndSendAlerts(data) {
  ...
  for (const coin of data) {
    ...
    if (!notifSetup || !notifSetup.entry) {
      ...
      if (_macroCache) {
        try {
          ...
          const _preRiskConf = Math.max(0, 100 - calcLearnDrag(notifSetup.learnPenalty || 0));
          try {
    ...
    // 每次都重新計算，避免快取中的舊等級（S→A 條件改變後）導致長線單建立後立即降格
    {
      try {
        ...
        if (_qH4 && _q15m) { // 硬性條件通過才計入 SQ
          ...
          // 日線僅長線單計分（短線單不看日線）
          if (notifSetup.isLongTerm === true) { if (_qDy) _qSq += 1; else if (_qDyOp) _qSq -= 1; }
        ...
        try {
          const _qPRConf=Math.max(0,100-calcLearnDrag(notifSetup.learnPenalty||0));
    ...
    const _notifPreRiskConf = Math.max(0, 100 - calcLearnDrag(notifSetup.learnPenalty || 0));
    ...
    try {
      ...
      if (!_alreadyIn && !_alertDirGuard && _alertLsgPass) {
        try {
          ...
          try {
            if (_macroCache) {
              _aCtx.wBias = computeWeeklyAIBias(_macroCache.fg, _macroCache)?.bias || 'neutral';
              _aCtx.tBias = computeTodayAIBias(_macroCache.fg, _macroCache)?.bias  || 'neutral';
            }
            _aCtx.btcChg24 = parseFloat(data?.find(d => d.symbol === 'BTC/USDT')?.change24h);
          } catch(_ce) {}
      ...
      if (!_alreadyIn && !_alertDirGuard && _alertLsgPass && _alertAuditPass) {
...
/* 風險評估扣分（比例制，2026-07 調降版）：高風險只扣風控分、不硬性擋單。
   低風險 <30 不扣；中風險 30-59 → 1~8；高風險 60-74 → 9~15；極高風險 75+ → 15~22 */
function calcRiskPenalty(score) {
  if (score < 30) return 0;
  if (score < 60) return Math.max(1, Math.round((score - 29) / 30 * 8));
  if (score < 75) return Math.max(9, Math.round(9 + (score - 59) / 15 * 6));
  return Math.min(22, Math.round(15 + (score - 74) / 26 * 7));
}
...
function computeSimpleSetup(coin, isLong) {
  ...
  const _ssLearnDrag = calcLearnDrag(learnPenalty);
